const testimonialSlider = function () {
	if (jQuery(window).width() > 992) {
		...
		jQuery(document)
			.find(".header-bar-menu > ul")
			.owlCarousel(testimonialConfig);
	}
};
...
function checkBox() {
	var headerCheckbox = jQuery(
		".cart-list-header .cart-list-header-check-all .stardust-checkbox"
	);
	var shopCheckbox = jQuery(
		".cart-list .cart-item-outer .cart-item-header-checkbox .stardust-checkbox"
	);
	var productCheckbox = jQuery(
		".cart-list .cart-item-outer .cart-item-body-item-checkbox .stardust-checkbox"
	);
	var footerCheckbox = jQuery(
		".cart-final--bottom-checkbox .stardust-checkbox"
	);
	...
	jQuery(document).on("change", ".stardust-checkbox__input", function (e) {
		...
		if (jQuery(this).is(":checked")) {
			jQuery(this)
				.closest(".stardust-checkbox")
				.addClass("stardust-checkbox--checked");
		} else {
			jQuery(this)
				.closest(".stardust-checkbox")
				.removeClass("stardust-checkbox--checked");
		}
	});
	jQuery(document).on(
		"change",
		".cart-list-header .cart-list-header-check-all .stardust-checkbox__input, .cart-final--bottom-checkbox .stardust-checkbox__input",
		function (e) {
			e.preventDefault();
			if (jQuery(this).is(":checked")) {
	...
	jQuery(document).on(
		"change",
		".cart-list .cart-item-outer .cart-item-header-checkbox .stardust-checkbox__input",
		function (e) {
			e.preventDefault();
			var thisShopCheckbox = jQuery(this)
				.parents(".cart-item-outer")
				.find(
					".cart-item-body .cart-item-body-item-checkbox .stardust-checkbox"
				);
			if (jQuery(this).is(":checked")) {
...
let Main = {
	...
	upEvent: function (container) {
		...
		container.find(".mwc-countdown-timer").each(function () {
			...
			let hoursTens =
				(360000 - (tMod % 360000) + (36000 - preTimeout)) * -1;
			...
			let hoursTensObj = obj.find(
				".mwc-countdown-timer__number__hexa--hour"
			);
			let hoursOnesObj = obj.find(
				".mwc-countdown-timer__number__deca--hour"
			);
			let minsTensObj = obj.find(
				".mwc-countdown-timer__number__hexa--minute"
			);
			let minsOnesObj = obj.find(
				".mwc-countdown-timer__number__deca--minute"
			);
			let secsTensObj = obj.find(
				".mwc-countdown-timer__number__hexa--second"
			);
			let secsOnesObj = obj.find(
				".mwc-countdown-timer__number__deca--second"
			);
	...
	onEvent: function () {
		...
		jQuery(document).on(
			"click",
			".cart-item-body-item-product-options-btn",
			function (e) {
				e.preventDefault();
				var optionsTop = jQuery(this)
					.parents(".cart-item-body-item-product-options")
					.offset().top;
				var optionsHeight = jQuery(this)
					.parents(".cart-item-body-item-product-options")
					.outerHeight();
				var parentTop = jQuery(this)
					.parents(".cart-item-body-item")
					.offset().top;
				var modalTop = optionsTop - parentTop;
				jQuery(this)
					.next()
					.toggleClass("cart-item--options-modal--active");
				jQuery(this)
					.next()
					.css("top", modalTop + optionsHeight);
			}
		);
		jQuery(document).on(
			"click",
			".cart-item--options-modal--actions button",
			function (e) {
				e.preventDefault();
				jQuery(this)
					.parents(".cart-item--options-modal")
					.removeClass("cart-item--options-modal--active");
			}
		);
		jQuery(document).on(
			"click",
			...
			function (e) {
				...
				var input = jQuery(this)
					.parent()
					.find(".cart-item--quantity-actions--input");
				var value = Number(input.val());
				if (
					jQuery(this).hasClass(
						"cart-item--quantity-actions--item-minus"
					)
				) {
				...
				if (
					jQuery(this).hasClass(
						"cart-item--quantity-actions--item-plus"
					)
				) {
					value = value + 1;
					input.val(value);
		...
		jQuery(document).on(
			...
			function (e) {
				...
				jQuery(this)
					.parent()
					.find(".cart-item--find-more--dropdown")
					.toggleClass("cart-item--find-more--dropdown--active");
			}
		);
		jQuery(document).on(
			"click",
		...
		jQuery(document).on("click", ".stardust-radio", function (e) {
			...
			var input = jQuery(this)
				.parents(".stardust-radio-group")
				.attr("data-trigger-value");
			jQuery(input).attr("data-value", value);
			jQuery(input).val(value);
			jQuery(this)
				.parent(".stardust-radio-group")
				.find(".stardust-radio")
				.removeClass("stardust-radio--checked");
			jQuery(this)
				.parent(".stardust-radio-group")
				.find(".stardust-radio-button")
				.removeClass("stardust-radio-button--checked");
			jQuery(this).addClass("stardust-radio--checked");
			jQuery(this)
				.find(".stardust-radio-button")
				.addClass("stardust-radio-button--checked");
		});
		...
		jQuery(document).on("click", ".custom-tab-nav-item", function (e) {
			...
			jQuery(this)
				.parent(".custom-tab-nav")
				.find(".custom-tab-nav-item")
				.removeClass("custom-tab-nav-item--active");
			jQuery(this).addClass("custom-tab-nav-item--active");
			jQuery(target)
				.parent(".custom-tab-contents")
				.find(".custom-tab-content-item")
				.removeClass("custom-tab-content-item--active");
		...
		jQuery(document).on(
			...
			function (e) {
				...
				var target = jQuery(this).attr("data-target");
				jQuery(this)
					.parent(".stardust-tabs-header")
					.find(".stardust-tabs-header__tab")
					.removeClass("stardust-tabs-header__tab--active");
				jQuery(this).addClass("stardust-tabs-header__tab--active");
				jQuery(target)
					.parent(".stardust-tabs-contents")
					.find(".stardust-tabs-content-item")
					.removeClass("stardust-tabs-content-item--active");
		...
		jQuery(document).on(
			...
			function (e) {
				e.preventDefault();
				jQuery(this)
					.parents(".account-noti-item")
					.find(".account-noti-item-body")
					.slideToggle();
				jQuery(this).toggleClass(
					"account-noti-item-dropdown-btn--active"
				);
			}
		);
	},
...
jQuery(window).resize(function () {
	jQuery(document)
		.find(".header-bar-menu")
		.find(".toggle-menu-sub")
